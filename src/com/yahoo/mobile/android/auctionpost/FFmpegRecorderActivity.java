...
import com.yahoo.mobile.android.auctionpost.videorecorder.CONSTANTS;
import com.yahoo.mobile.android.auctionpost.videorecorder.NewFFmpegFrameRecorder;
import com.yahoo.mobile.android.auctionpost.videorecorder.ProgressView;
import com.yahoo.mobile.android.auctionpost.videorecorder.ProgressView.State;
import com.yahoo.mobile.android.auctionpost.videorecorder.RecorderParameters;
import com.yahoo.mobile.android.auctionpost.videorecorder.SavedFrames;
import com.yahoo.mobile.android.auctionpost.videorecorder.Util;
...
public class FFmpegRecorderActivity extends Activity implements OnClickListener {
    ...
    ImageButton flashIcon = null, cancelBtn, nextBtn, switchCameraIcon = null;

    /* add by Jean */
    ImageButton recordingBtn, switchToCameraBtn;
    private boolean shouldBeRecorded = false;
    private RelativeLayout mProgressLayout;
	private ProgressBar mVideoProcessingProgress;
    ...
    /*
     * private Runnable mUpdateTimeTask = new Runnable() { public void run() { if(rec) setTotalVideoTime(); handler.postDelayed(this, 200); } };
     */
    ...
    // 录制的最长时间
    public static int recordingTime = 15000;
    // 录制的最短时间
    private int recordingMinimumTime = 6000;
    // 提示换个场景
    private int recordingChangeTime = 15000;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("activity", "FFmpegRecorderActivity onCreate");
        ...
        Log.d("scale", "screenWidth=" + screenWidth + "screenHeight=" + screenHeight);
...
//			Intent intent = new Intent(FFmpegRecorderActivity.this, StreamActivity.class);
//			startActivity(intent);
	...
	private void initLayout()
	{
		...
		/* add by Jean */
		recordingBtn = (ImageButton) findViewById(R.id.btn_take_video);
		recordingBtn.setOnClickListener(this);
		switchToCameraBtn = (ImageButton) findViewById(R.id.btn_switch_to_camera);
		switchToCameraBtn.setOnClickListener(this);
		
		RelativeLayout panelLayout = (RelativeLayout) findViewById(R.id.video_panel_container);
...
//		}
		...
		mProgressLayout = (RelativeLayout) findViewById(R.id.loadingPanel_video);
        mVideoProcessingProgress = (ProgressBar) findViewById(R.id.progress_bar_video_processing);
		...
		mVideoProcessingProgress.setVisibility(View.VISIBLE);
	...
	private void initCameraLayout() {
		new AsyncTask<String, Integer, Boolean>(){
			...
			protected void onPostExecute(Boolean result) {
				...
				mTopLayout = (RelativeLayout) findViewById(R.id.recorder_surface_parent);
				if(mTopLayout != null && mTopLayout.getChildCount() > 0)
				...
				mTopLayout.addView(cameraView, layoutParam1);
				
//				switchCameraIcon.setOnClickListener(FFmpegRecorderActivity.this);
				mVideoProcessingProgress.setVisibility(View.GONE);
			}
	...
	public class AsyncStopRecording extends AsyncTask<Void,Integer,Void>{
		...
		// cut to 480*480 && rotate 90 degrees
		private void getFirstCapture(byte[] data){
			...
			try {
				...
				Log.d("size", "FFmpeg first frame width="+localBitmap2.getWidth()+" height="+localBitmap2.getHeight());
	...
	private void showCancellDialog(){
		Util.showDialog(FFmpegRecorderActivity.this, "提示", "確定要放棄這段影片？", 2, new Handler(){
			@Override
			public void dispatchMessage(Message msg) {
				if(msg.what == 1)
					videoTheEnd(false);
	...
	class CameraView extends SurfaceView implements SurfaceHolder.Callback, PreviewCallback {
		...
		private boolean startContinuousAutoFocus() {
...
//					nextBtn.setEnabled(true);
	...
	private void handleSurfaceChanged()
	{
		...
		// extract all supported resolution
		...
		if(resolutionList != null && resolutionList.size() > 0){
			...
			if(defaultScreenResolution == -1){
				...
				// forced resolution set to 640*480 if camera resolution is supported
			...
			if(previewSize != null ){
				...
				Log.d("scale", "previewWidth=" + previewWidth + "previewHeight" + previewHeight);
	...
	public void onClick(View v) {
		...
		if (v.getId() == R.id.btn_take_video) {
			if(!recordFinish){
				if (shouldBeRecorded == true) {
					recordingBtn.setImageDrawable(getResources().getDrawable(R.drawable.btn_record));
					...
					recordingBtn.setImageDrawable(getResources().getDrawable(R.drawable.btn_stop_recording));
					mHandler.removeMessages(3);
					mHandler.removeMessages(4);
		...
		} else if (v.getId() == R.id.btn_switch_to_camera) {
			...
			Intent intent = new Intent(FFmpegRecorderActivity.this, PhotoTakingActivity.class);
			Log.d("activity", "in FFmpegRecorderActivity: previewWidth = " + previewWidth + "previewHeight = " + previewHeight);
			startActivity(intent);
			finish();
			return;
		} 
	...
	private void returnToCaller(boolean valid)
	{
		try{
			...
			if(valid){
				Intent intent = new Intent(this, MediaDecorateActivity.class);
				intent.putExtra("videoPath", strVideoPath);
				intent.putExtra("videoUri", (Uri.parse("file://"+strVideoPath)).toString());
				intent.putExtra("imagePath", imagePath);
				intent.putExtra("imageUri", (Uri.parse("file://"+imagePath)).toString());
				
				Log.d("path", "FFmpeg imagePath:"+imagePath);
				Log.d("uri", "FFmpeg imageUri:" + Uri.parse(imagePath));
				Log.d("path", "FFmpeg videoPath:"+strVideoPath);
				Log.d("path", "FFmpeg videoPath:"+uriVideoPath.getPath());
				Log.d("uri", "FFmpeg videoUri:" + uriVideoPath);
	...
	// register in system to store video
